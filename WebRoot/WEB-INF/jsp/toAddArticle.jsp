			...
			<c:choose>
			    ...
			    <c:when test="${user!=null && user.userpermission==0 || user.userpermission==1}">
				    <form  id="addArticle" role="form" method="post" onsubmit="return false">
		        		<button class="btn btn-primary" type="submit" onclick="addArticle();" style="float:right">提交</button><br>
		        		<input type="hidden" id="user_id" name="user_id" value="${user.id}"><br>
		        		<input class="form-control" type="text" name="title" placeholder="请输入文章标题"><br>
						...
						<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/ueditor/third-party/video-js/video-js.min.css"/>  
    					<script type="text/javascript" src="${pageContext.request.contextPath}/ueditor/third-party/video-js/video.js"></script>  
    					<script type="text/javascript" src="${pageContext.request.contextPath}/ueditor/ueditor.parse.js"></script>
						<script id="summary" name="summary" type="text/plain" style="height:300px"></script>
		        		<label>文章主要内容:</label>
